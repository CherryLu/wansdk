...
//		TODO 设置浏览尺寸
        ...
        Log.e("recondbase", ""+findWidth);
        if (!findWidth) {
            Log.e("recondbase", "传入高度不支持或未找到对应宽度,请按照要求重新设置，否则会出现一些严重问题");

            int nearHeight = RecordUtil.getCloselyMun(SMALL_VIDEO_HEIGHT,mSupportedPreviewSizes);
            if(mSupportedPreviewSizes.contains(nearHeight)){

                int index = mSupportedPreviewSizes.indexOf(nearHeight);
                mSupportedPreviewWidth = mSupportedPreviewSizes.get(index).width;
                SMALL_VIDEO_HEIGHT = mSupportedPreviewSizes.get(index).height;
                Log.e("recondbase", ""+nearHeight+" "+mSupportedPreviewWidth+" "+SMALL_VIDEO_HEIGHT);
                checkFullWidth(mSupportedPreviewWidth,SMALL_VIDEO_WIDTH);
            }else {
...
////        if(context !=null){
//            int w = DeviceUtils.getScreenHeight(context);
//            int h = DeviceUtils.getScreenWidth(context);
            Log.e("setPreviewSize", mSupportedPreviewWidth+" "+SMALL_VIDEO_HEIGHT);
////            mParameters.setPreviewSize(SMALL_VIDEO_HEIGHT*w/h, SMALL_VIDEO_HEIGHT);
////        }else {
//        int wt = SMALL_VIDEO_HEIGHT*h/w;
//        Log.e("setPreviewSize",wt+"");
            mParameters.setPreviewSize(mSupportedPreviewWidth, SMALL_VIDEO_HEIGHT);
//        }

//        mParameters.setPictureSize(mSupportedPreviewWidth, SMALL_VIDEO_HEIGHT);

        // 设置输出视频流尺寸，采样率
        mParameters.setPreviewFormat(ImageFormat.YV12);//YV12NV21
    ...
    protected Boolean doCompress(boolean mergeFlag) {
        if (compressConfig != null) {
            ...
            String cmd_transcoding = String.format("ffmpeg -threads 16 -i %s -c:v libx264 %s %s %s -c:a libfdk_aac %s %s %s %s",
                    mMediaObject.getOutputTempVideoPath().replace(" ","zgbkkk"),
