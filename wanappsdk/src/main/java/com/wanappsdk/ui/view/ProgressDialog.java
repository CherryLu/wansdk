...
import com.wanappsdk.utils.OssUtils;
...
public class ProgressDialog extends Dialog implements OssUtils.ProgressCallback {
            ...
            switch (msg.what){
                ...
                case 201:
                    ...
                    if (currentPosition<files.size()){//继续上传
                        ...
                        ossUtils.beginupload(getContext(), policyId,UserManager.getInstance().getId()+"="+id+"="+currentPosition,files.get(currentPosition));
                    }else {//上传结束 上报服务器
                        LogUtils.e("ZX","结束");
    ...
    public ProgressDialog(Context context, List<String> pics, String id,String policyId) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currentPosition = 0;
        File file = new File(files.get(currentPosition));
        ...
        ossUtils.beginupload(getContext(), policyId,UserManager.getInstance().getId()+"="+id+"="+currentPosition+"-"+name+"-"+file.length(),files.get(currentPosition));
    ...
    public void onSuccess(String name) {
        ...
        sb.append(";");
        LogUtils.d("onSuccess",sb.toString());
        if (handler!=null){
